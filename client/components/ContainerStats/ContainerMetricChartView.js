...
export default class ContainerMetricChartView extends React.Component {
  ...
  componentDidMount() {
    ...
    cpuData.labels = this.state.containers.map(container => container.container_name)
    cpuData.datasets[0].data = this.state.containers.map(container => container.cpu_usage)
    cpuData.datasets[0].backgroundColor = backgroundColors        

    memoryData.labels = cpuData.labels
    memoryData.datasets[0].data = this.state.containers.map(container => container.memory_usage)
    memoryData.datasets[0].backgroundColor = backgroundColors
  }

  render() {
    return <div style={styles.flex_wrapper}>
      <div> <Pie options={cpuOption} data={cpuData} /></div>
      <div> <Pie options={memoryOption} data={memoryData} /></div>
    </div>
  }
